...
impl PaneLayout {
        ...
        match self {
            PaneLayout::Leaf(id) if id == target => {
                let (first, second) = if new_is_second {
                    (Box::new(PaneLayout::Leaf(id)), Box::new(PaneLayout::Leaf(new)))
                } else {
                    (Box::new(PaneLayout::Leaf(new)), Box::new(PaneLayout::Leaf(id)))
                };
                PaneLayout::Split { dir, ratio: 0.5, first, second }
            }
            PaneLayout::Split { dir: d, ratio, first, second } => PaneLayout::Split {
                dir: d,
                ratio,
    ...
    pub fn remove(self, target: PaneId) -> Option<Self> {
        match self {
            PaneLayout::Leaf(id) => {
                if id == target { None } else { Some(PaneLayout::Leaf(id)) }
            }
            PaneLayout::Split { dir, ratio, first, second } => {
                match (first.remove(target), second.remove(target)) {
                    ...
                    (Some(f), None) => Some(f),
                    (Some(f), Some(s)) => {
                        Some(PaneLayout::Split { dir, ratio, first: Box::new(f), second: Box::new(s) })
                    }
                    (None, None) => None,
                }
            }
    ...
    pub fn set_ratio(&mut self, path: &[bool], new_ratio: f32) {
        match self {
            PaneLayout::Split { ratio, first, second, .. } => {
                if path.is_empty() {
...
pub fn zone_to_split(zone: DropZone, dragged: PaneId, target: PaneId) -> Option<SplitAction> {
    match zone {
        DropZone::Left => {
            Some(SplitAction { target, new: dragged, dir: SplitDir::Horizontal, new_is_second: false })
        }
        DropZone::Right => {
            Some(SplitAction { target, new: dragged, dir: SplitDir::Horizontal, new_is_second: true })
        }
        DropZone::Top => {
            Some(SplitAction { target, new: dragged, dir: SplitDir::Vertical, new_is_second: false })
        }
        DropZone::Bottom => {
            Some(SplitAction { target, new: dragged, dir: SplitDir::Vertical, new_is_second: true })
        }
...
pub fn split_rect(rect: Rect, dir: SplitDir, ratio: f32) -> (Rect, Rect) {
    ...
    match dir {
        SplitDir::Horizontal => {
            ...
            let first = Rect::from_min_max(rect.min, egui::pos2(split_x - DIVIDER / 2.0, rect.max.y));
            let second = Rect::from_min_max(egui::pos2(split_x + DIVIDER / 2.0, rect.min.y), rect.max);
            (first, second)
        }
        SplitDir::Vertical => {
            let split_y = rect.top() + rect.height() * ratio;
            let first = Rect::from_min_max(rect.min, egui::pos2(rect.max.x, split_y - DIVIDER / 2.0));
            let second = Rect::from_min_max(egui::pos2(rect.min.x, split_y + DIVIDER / 2.0), rect.max);
            (first, second)
        }
    }
}
